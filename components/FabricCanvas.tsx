...
export default function FabricCanvas({
  ...
  // Masking/Inpainting states
  const [isDrawingMode, setIsDrawingMode] = useState(false);
  const [maskBrush, setMaskBrush] = useState<fabric.PencilBrush | null>(null);
  const [brushSize, setBrushSize] = useState(30);
  const [showInpaintModal, setShowInpaintModal] = useState(false);
  const [inpaintPrompt, setInpaintPrompt] = useState("");
  const [inpaintingLayer, setInpaintingLayer] = useState<string | null>(null);
  ...
  async function handleInpaintLayer(layerId: string) {
    setInpaintingLayer(layerId);
  ...
  function clearMask() {
    ...
    // Remove all path objects (drawn mask)
  ...
  async function handleInpaint() {
    if (!fabricRef.current || !inpaintPrompt.trim()) return;

    const canvas = fabricRef.current;

    // Check if we're inpainting a specific layer
    const targetLayer = inpaintingLayer ? layers.find((l) => l.id === inpaintingLayer) : null;

    try {
      setIsProcessing(true);
      setProcessingMessage(targetLayer ? `Inpainting ${targetLayer.name}...` : "Creating inpainted image...");

      // Translate prompt if needed
      let englishPrompt = inpaintPrompt;
      const hasTurkish = /[ğüşıöçĞÜŞİÖÇ]/.test(inpaintPrompt);

      if (hasTurkish) {
        console.log("🌐 Translating inpaint prompt to English...");
        try {
          const translateRes = await fetch('/api/translate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          const translateData = await translateRes.json();
          englishPrompt = translateData.translated || inpaintPrompt;
          console.log("✅ Translated:", englishPrompt);
        } catch (err) {
      ...
      if (targetLayer) {
        // Layer-specific inpainting: export only the layer with mask
        const obj = targetLayer.object as fabric.Image;
        compositeDataURL = obj.toDataURL({ format: "png" });

        // Create mask from drawn paths relative to layer bounds
        ...
        // Draw paths in white, translated to layer coordinates
        const paths = canvas.getObjects().filter((obj) => obj.type === "path");
        if (paths.length === 0) {
          alert("Please draw a mask on the layer first!");
        ...
        paths.forEach((path) => {
          const pathObj = path as fabric.Path;
          const pathPoints: any = pathObj.path;

          if (pathPoints && Array.isArray(pathPoints)) {
            maskCtx.beginPath();
            pathPoints.forEach((point: any) => {
              if (point[0] === 'M') {
                maskCtx.moveTo(point[1] - bounds.left, point[2] - bounds.top);
              } else if (point[0] === 'L') {
                maskCtx.lineTo(point[1] - bounds.left, point[2] - bounds.top);
              } else if (point[0] === 'Q') {
        ...
        console.log("🎭 Layer-specific mask created for", targetLayer.name);
      } else {
        // Full canvas inpainting
        compositeDataURL = canvas.toDataURL({ format: "png", quality: 1.0 });

        // Check for drawn paths
        const paths = canvas.getObjects().filter((obj) => obj.type === "path");
        if (paths.length === 0) {
        ...
        // Create a temporary fabric canvas for mask rendering
        ...
        // Clone and add all paths in white
        for (const path of paths) {
          const clonedPath = await new Promise<fabric.Path>((resolve) => {
            (path as fabric.Path).clone((cloned: fabric.Path) => {
              cloned.set({
                stroke: "#FFFFFF",
                strokeWidth: (path as fabric.Path).strokeWidth,
                fill: "",
              });
              resolve(cloned);
            });
          });
          tempFabricCanvas.add(clonedPath);
        }

        tempFabricCanvas.renderAll();
        maskDataURL = tempFabricCanvas.toDataURL({ format: "png", quality: 1.0 });
        tempFabricCanvas.dispose();
        console.log("🎭 Full canvas mask created");
      }
      ...
      const [compositeBlob, maskBlob] = await Promise.all([
        (await fetch(compositeDataURL)).blob(),
        (await fetch(maskDataURL)).blob(),
      ]);
      ...
      // Debug: Log mask size
      console.log("🎭 Mask created, size:", maskBlob.size);

      // Call inpainting API
      const formData = new FormData();
      formData.append("image", compositeFile);
      ...
      while (!completed && Date.now() - startTime < 180000) {
        ...
        const statusRes = await fetch(`/api/inpaint/status?prompt_id=${promptId}&_=${Date.now()}`, {
          cache: "no-store",
        });
        const statusData = await statusRes.json();

        if (statusData.completed && statusData.images.length > 0) {
          console.log("✅ Inpainting completed:", statusData.images[0]);
          ...
          // Replace background layer with inpainted result
          fabric.Image.fromURL(imgUrl, (img) => {
            // Find and remove the background layer
            const bgLayer = layers.find((l) => l.id === "bg-layer");
            if (bgLayer) {
              canvas.remove(bgLayer.object);
            }

            // Add new inpainted image as background
            const layerId = "bg-layer";
            img.scaleToWidth(canvas.width!);
            img.scaleToHeight(canvas.height!);
            img.selectable = false;
            ...
            // Update layers list
            const updatedLayers = layers.filter((l) => l.id !== "bg-layer");
            updatedLayers.unshift({
              id: layerId,
              name: "Background (Inpainted)",
              object: img,
              visible: true,
            });
            setLayers(updatedLayers);

            // Clear the mask paths
            const pathsToRemove = canvas.getObjects().filter((obj) => obj.type === "path");
            pathsToRemove.forEach((path) => canvas.remove(path));

            canvas.renderAll();

            setShowInpaintModal(false);
            setInpaintPrompt("");
            alert("✅ Inpainting completed! Background updated.");
          });

          completed = true;
        } else if (statusData.status === "error") {
          throw new Error("Inpainting failed in ComfyUI");
        }
      }
  ...
  return (
    <div className="flex gap-3 relative">
        ...
        <div className="space-y-2">
          ...
          <div className="bg-gray-800 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-white font-semibold text-sm">🖌️ Inpainting</h4>
              {isDrawingMode && (
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                {isDrawingMode ? "✓ Stop Drawing" : "🖌️ Draw Mask"}
              </button>
              ...
              {isDrawingMode && (
                <>
                  ...
                  <div className="flex items-center gap-2 bg-gray-700 px-3 py-2 rounded-lg">
                    <span className="text-white text-xs">Brush:</span>
                    <button
                      onClick={() => changeBrushSize(10)}
                      className={`px-2 py-1 rounded text-xs ${
                        brushSize === 10 ? "bg-orange-600 text-white" : "bg-gray-600 text-gray-300"
                      }`}
                    >
                      S
                    </button>
                    <button
                      onClick={() => changeBrushSize(30)}
                      className={`px-2 py-1 rounded text-xs ${
                        brushSize === 30 ? "bg-orange-600 text-white" : "bg-gray-600 text-gray-300"
                      }`}
                    >
                      M
                    </button>
                    <button
                      onClick={() => changeBrushSize(60)}
                      className={`px-2 py-1 rounded text-xs ${
                        brushSize === 60 ? "bg-orange-600 text-white" : "bg-gray-600 text-gray-300"
                      }`}
                    >
                      L
                    </button>
              ...
              <button
                ...
                className="px-3 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-semibold rounded-lg text-sm"
              >
                ✨ Apply Inpaint
              </button>
            </div>
            ...
            <p className="text-gray-400 text-xs">
              💡 Draw on the area you want to change, then apply inpainting with a prompt
            </p>
          </div>
        </div>
                  ...
                  {layer.id !== "bg-layer" && (
                    <>
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleInpaintLayer(layer.id);
                        }}
                        className="text-orange-400 hover:text-orange-300 text-xs px-1"
                        title="Inpaint Layer"
                      >
                        🖌️
                      </button>
      ...
      {showInpaintModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-white font-bold mb-3">Inpaint Masked Area</h3>
            <p className="text-gray-400 text-xs mb-3">
            ...
            <textarea
              ...
              placeholder="e.g., 'pink fluffy slippers', 'sunglasses', 'red dress'"
              className="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 mb-4"
              rows={3}
              autoFocus
              disabled={isProcessing}
            />
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-semibold rounded-lg text-sm flex items-center justify-center gap-2"
              >
                {isProcessing ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
